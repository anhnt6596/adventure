...
// "Which items are near this point" as a uniform grid. A data structure, not a system: collision
// and combat each own one, so nothing that only collides ends up in a combat query.
public class SpatialHash<T>
{
    ...
    public void Rebuild()
    {
        ...
        for (int i = 0; i < _items.Count; i++)
            BucketOf(_positionOf(_items[i]), true).Add(i);
    }
    ...
    List<int> BucketOf(Vector3 p, bool create)
    {
